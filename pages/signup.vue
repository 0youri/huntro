...
<script setup lang="ts">
  ...
  const validate = (state: any): FormError[] => {
    ...
    if (!state.passwordConfirmation) errors.push({ name: 'passwordConfirmation', message: 'Required' })
    if (state.password !== state.passwordConfirmation) errors.push({ name: 'passwordConfirmation', message: 'Passwords do not match' })
    return errors
  }
